...
export const authMiddleware = createMiddleware<{ Variables: { user: JwtPayload } }>(
  async (c, next) => {
    ...
    try {
      ...
      c.set('user', {
        ...
        name: typeof decodedPayload.name === 'string' || decodedPayload.name === null ? decodedPayload.name : undefined,
      } as JwtPayload);
    } catch (error) {
