...
describe('Auth Routes (/api/v1/auth)', () => {
  ...
  describe('POST /login', () => {
    ...
    it('should return 401 if email does not exist', async () => {
      const res = await app.request('/api/v1/auth/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: 'wrong@example.com', password: testUser.password }),
      });
    ...
    it('should return 401 if password is incorrect', async () => {
     ...
     it('should return 400 if email is invalid', async () => {
      const res = await app.request('/api/v1/auth/login', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: 'invalidemail', password: testUser.password }),
      });
      expect(res.status).toBe(400);
    });
  });
  ...
  describe('GET /me', () => {
    ...
    beforeEach(async () => {
      ...
      const dbUser = await prisma.user.findUnique({where: {email: testUser2.email}});
