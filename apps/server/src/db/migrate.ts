...
async function ensureDatabaseExists(dbConfig: URL) {
  ...
  try {
    ...
    logger.log(
      `Connected to '${tempDbConfig.pathname.slice(1)}' database to check for '${dbName}'.`,
    );
    ...
    logger.error(
      `Error during database existence check or creation for '${dbName}':`,
      error,
    );
    process.exit(1);
  } finally {
    await client.end();
    logger.log(
      `Disconnected from '${tempDbConfig.pathname.slice(1)}' database.`,
    );
  }
}
