  ...
  return (
    <div className="window-titlebar-menu">
      {menuItems?.map((menu, index) => (
        <TitlebarMenuItem key={menu.name} menu={menu} index={index} />
      ))}
    </div>
  );
};
...
const TitlebarMenuItem = ({
  menu,
  index,
}: { menu: TitlebarMenu; index: number }) => {
      ...
      <div
        ...
        onKeyDown={(e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
            // biome-ignore lint/suspicious/noExplicitAny: KeyboardEvent to MouseEvent conversion for consistent handler
  ...
  return (
    <div className="menuItem-popup">
      {menu.items.map((item, index) => (
        <TitlebarMenuPopupItem key={`${item.name}-${index}`} item={item} />
      ))}
    </div>
  );
};
