...
function fixTestAnyErrors() {
  ...
  try {
    ...
    const checkResult = execSync(
      "npx biome check --max-diagnostics=1000 2>&1",
      {
        encoding: "utf8",
        shell: true,
      },
  ...
  console.log(
    `Found ${anyErrors.length} noExplicitAny errors, processing test files...`,
  );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.includes("lint/suspicious/noExplicitAny")) {
      ...
      const match = line.match(
        /^(.+):(\d+):(\d+)\s+lint\/suspicious\/noExplicitAny/,
      );
      if (match) {
  ...
  for (const error of errors) {
    ...
    if (line && line.includes("as any")) {
      ...
      if (
        ...
        console.log(
          `  Line ${error.lineNumber}: biome-ignore comment already exists, skipping`,
        );
        continue;
      }
      ...
      if (line.includes("biome-ignore lint/suspicious/noExplicitAny")) {
        console.log(
          `  Line ${error.lineNumber}: biome-ignore comment already exists on same line, skipping`,
        );
        continue;
      }
      ...
      console.log(
        `  Fixed line ${error.lineNumber}: added biome-ignore comment`,
      );
    }
  }
